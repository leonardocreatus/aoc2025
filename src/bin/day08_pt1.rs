...
fn main() {
    ...
    let mut distances_per_points: HashMap<u32, Vec<(Point, Point)>> = HashMap::new();
    ...
    while i < 1000 {
        ...
        for (p1, p2) in points {
            ...
            match (idx_a, idx_b) {
                ...
                (Some(ia), None) => {
                    circuits[ia].insert(*p2);
                }
                (None, Some(ib)) => {
                    circuits[ib].insert(*p1);
                }
                (None, None) => {
    ...
    let mut len_circuits: Vec<usize> = circuits.iter().map(|c| c.len()).collect();
    len_circuits.sort();
